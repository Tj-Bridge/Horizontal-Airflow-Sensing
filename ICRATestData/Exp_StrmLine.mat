...
% For griddata, you need points where you know the flow values
...
[startX, startY] = meshgrid(linspace(min(x), max(x), 20), linspace(min(y), max(y), 20));

% Plot Data
size(flow_Yinterpolated)
size(flow_Xinterpolated)
size(x')
size(y')
